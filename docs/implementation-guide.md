# PCF Template — Implementation Guide

A reference for every utility, hook, service, and pattern in this template. Copy the patterns here verbatim for new controls.
...
File: `PCFTemplate/services/DataverseService.ts`
...
| App models | `model/app/` | `PCF_` | Used by all React code |
...
File: `PCFTemplate/hooks/useOutputs.ts`
...
File: `PCFTemplate/hooks/useControlValue.ts`
...
File: `PCFTemplate/hooks/useControlContext.ts`
...
File: `PCFTemplate/utils/date.ts`
...
File: `PCFTemplate/utils/guid.ts`
...
File: `PCFTemplate/utils/index.ts`
...
File: `PCFTemplate/types/index.ts`
...
```ts
const [state, setState] = React.useState<AsyncState<PCF_Opportunity[]>>({
    data: null,
    isLoading: false,
    error: null,
});
...
Files:
- `strings/PCFTemplate.1033.resx` — English (base / fallback)
- `strings/PCFTemplate.<LCID>.resx` — additional locales (e.g. `2057` = en-GB, `1031` = de-DE)

The manifest must declare the resource:
```xml
<resx path="strings/PCFTemplate.1033.resx" version="1.0.0" />
```

When adding a new locale, duplicate `PCFTemplate.1033.resx`, rename it with the new LCID, translate the `<value>` entries, and add a second `<resx>` line in the manifest for that LCID.
...
| Key | Default value |
|---|---|
| `PCFTemplate_Name` | PCF Template |
| `PCFTemplate_Desc` | PCF Template control |
...
File: `PCFTemplate/hooks/useLocalisation.ts`
...
1. Duplicate `strings/PCFTemplate.1033.resx` → `strings/PCFTemplate.2057.resx`
2. Translate every `<value>` element
3. Add to the manifest:
   ```xml
   <resx path="strings/PCFTemplate.2057.resx" version="1.0.0" />
   ```
...
File: `PCFTemplate/utils/localisation.ts` — also re-exported from `utils/index.ts`.
...
| `@keyframes` animations | CSS file (`css/PCFTemplate.css`) |
...
| Class | Description |
|---|---|
| `.pcf-template-root` | Apply to the outermost container div. Resets `box-sizing`, sets `height/width: 100%`, enables `overflow: auto`. |
...
import "./css/PCFTemplate.css";

<div className="pcf-template-root">
  ...
</div>
